...
export async function GET(request: NextRequest) {
  ...
  const results = mockAgents.slice(start, end);

  return NextResponse.json({
    success: true,
    count: total,
    next: end < total ? `http://localhost:3000/api/v1/agents/?page=${page + 1}&page_size=${page_size}` : null,
    previous: page > 1 ? `http://localhost:3000/api/v1/agents/?page=${page - 1}&page_size=${page_size}` : null,
    results
  });
}
